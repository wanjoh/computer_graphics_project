...
vec3 CalcPointLight(PointLight light, vec3 normal, vec3 fragPos, vec3 viewDir, vec3 light_contribution);
...
void main()
{
    ...
    result += CalcPointLight(explosionLight, normal, FragPos, viewDir, vec3(0.2, 6.0, 2.0));
...
vec3 CalcPointLight(PointLight light, vec3 normal, vec3 fragPos, vec3 viewDir, vec3 light_contribution)
{
    ...
    vec3 diffuse = light.diffuse * diff * vec3(texture(material.texture_diffuse1, TexCoords));
    vec3 specular = light.specular * spec * vec3(texture(material.texture_specular1, TexCoords));
    ambient *= attenuation * light_contribution.x;
    diffuse *= attenuation * light_contribution.y;
    specular *= attenuation * light_contribution.z;
